...
@Component({
  selector: 'app-group-ticket-info',
  templateUrl: './group-ticket-info.component.html',
  styleUrls: ['./group-ticket-info.component.css']
})
export class GroupTicketInfoComponent implements OnInit, OnDestroy {
  form!: FormGroup;
  ...
  onBuy() {
    ...
    if (this.form.get('number')?.errors?.['required']){
      this.error_msg = 'Mora biti popunjeno polje za proj posetilaca!';
      return;
    } else if (this.form.get('number')?.value === 1) {
      this.error_msg = 'Za grupnu ulaznicu treba vise od jednog posetioca!';
      return;
    } else if (this.form.get('promoCode')?.value === '' ||
      this.form.get('promoCode')?.value === null ||
      this.form.get('promoCode')?.value === '54321') {
      this.visitorService.buyTicket(this.id, this.form.get('number')?.value, this.form.get('promoCode')?.value);
    } else {
